...
function add_order(item, order, source, quant) {
  // Generate a unique ID for the order so no overwriting occurs
...
function order_items(menu, orders, category) {
  ...
  do { 
    ...
    if (choice == 'yes') {
      ...
      } while (!(itemChoice in menu)); // Repeat until a valid item ID is entered
    }
  } while (choice != 'yes' && choice != 'no'); // Repeat until the user chooses yes or no
}
...
function place_order(order) {
  ...
  do {
    ...
    if (finished == 'yes') {
      ...
      if (tipChoice == 'yes') {
        tipAmount = parseFloat(prompt('How much would you like to tip in decimal? [15% = .15]: '));
        totalPrice += subtotalPrice*tipAmount;
      }
      let taxAmount = totalPrice*tax;
      // Adding tax after tip to simulate real world scenarios
      ...
      let payment;
      do {
        payment = prompt('Card or Cash? Card has a 5% fee. [Card/Cash]: ').toLowerCase();
