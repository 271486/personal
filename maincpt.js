...
function place_order(order) {
  ...
  if (finished == "yes") {
    ...
    console.log(`Subtotal: $${priceT.toFixed(2)}`);
    let tax =(parseInt(prompt("What is your state sales tax in percent? [6% = 6]: "))+100) / 100;
    let method = prompt("Pick-up or Delivery? Delivery has a $5 fee. [Pick-up/Delivery]: ").toLowerCase();
    let payment = prompt("Card or Cash? Card has a 5% fee. [Card/Cash]: ").toLowerCase();
    let tipChoice = prompt("Do you want to leave a tip? [Yes/No]: ").toLowerCase();
    if (tipChoice == "yes") {
      let tipAmount = (parseInt(prompt("How much you would like to tip in percent? [15% = 15]: ")) +100) /100;
      priceT *= tipAmount;
    }
    if (method == "delivery") {
      priceT += 5;
    ...
    priceT *= tax;
    console.log(`Total Cost: $${priceT.toFixed(2)}`);
  }
}
main_function();
